...
class HouseholdServiceService:
    @staticmethod
    def get_by_id(service_id):
        """
        Get household service by ID
        """
        return HouseholdRepository.get_by_id(service_id)
    
    @staticmethod
    def get_all():
        """
        Get all available household services
        """
        return HouseholdRepository.get_all()
    
    @staticmethod
    def get_by_provider(provider_id):
        """
        Get household services by provider
        """
        return HouseholdRepository.get_by_provider(provider_id)
    
    @staticmethod
    def get_by_service_type(service_type):
        """
        Get household services by type (maid, plumbing, etc.)
        """
        return HouseholdRepository.get_by_service_type(service_type)
    
    @staticmethod
    def create_household_service(provider_id, data):
        """
        ...
        # Create service
        service = HouseholdRepository.create(data, provider_id)
        if not service:
            return None, "Failed to create household service"
        
        return service, "Household service created successfully"
    
    @staticmethod
    def update_household_service(provider_id, service_id, data):
        """
        Update household service details
        """
        # Get service
        service = HouseholdRepository.get_by_id(service_id)
        if not service:
        ...
        # Update service
        updated_service = HouseholdRepository.update(service, data)
    ...
    @staticmethod
    def book_household_service(user_id, service_id, booking_date, amount, notes=None):
        """
        ...
        try:
            ...
            db.session.commit()
            return booking, "Household service booked successfully"
        except Exception as e:
            db.session.rollback()
            logger.error(f"Error booking household service: {str(e)}")
            return None, "Failed to book household service"
    
    @staticmethod
    def cancel_booking(user_id, booking_id):
        """
        Cancel a household service booking
        """
        # Get booking
    ...
    def update_booking_status(provider_id, booking_id, new_status):
        ...
        # Get service
        service = HouseholdRepository.get_by_id(booking.service_id)
        if not service:
